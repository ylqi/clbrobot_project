    ...
    heading_(0)
{
    ...
    velocity_subscriber_ = nh_.subscribe("raw_vel", 100, &ClbBase::velCallback, this);
    battery_sub_ = nh_.subscribe("battery", 0.2, &ClbBase::batteryCallback, this);
    nh_private.getParam("linear_scale", linear_scale_);
    nh_private.getParam("low_battery", low_battery_);
    
    //ROS_INFO("linear_scale_: %f", linear_scale_);
}

void ClbBase::batteryCallback(const clb_msgs::Battery& bat)
{
...
void ClbBase::velCallback(const clb_msgs::Velocities& vel)
{
